...
const AnnotatedDocumentViewer = ({
  ...
  const getMentionKey = (mention: Mention, index: number): string => {
    return `${mention.type}-${mention.content}-${index}`;
  };

  if (document.text) {
    let documentTextBlock = React.createElement(
      "div",
      { className: "annotated-document" },
      document.text
    );
    let mentionRows;
    if (document.mentions) {
      mentionRows = document.mentions.map((mention, index) => (
        <tr key={getMentionKey(mention, index)}>
          <td>
          ...
          <td>
            {mention.metadata &&
              displayMetadata(mention.type, mention.metadata)}
          </td>
          <td>
